...
story += [simple_steps([
    ...
    ("4. Choose future action years", "The Future Action Years selector controls the scheduled move rows shown after the coverage tables. Coverage rows are always shown from the account's Short-Term and Intermediate windows. Five years means the next five withdrawal years after that coverage; Full Plan shows the whole remaining ladder."),
...
story += [
    ...
    p("Intermediate Bucket amounts are labeled by Withdrawal Year, which means the future year the dollars are ultimately meant to cover. Refill ladders also show Withdrawal Year first, then a plain instruction. Example: with a 2-year Short-Term window and 2027 as the first operating year, the 2027 start-of-year Short-Term bucket covers 2027-2028. The 2029 withdrawal should move to Short-Term by the start of 2028, because the 2028 Short-Term bucket covers 2028-2029. A later Intermediate refill might say: 2034 withdrawal - stage from Long-Term into Intermediate by start of 2028 - move to Short-Term by start of 2033. The app does not recommend specific investments, but users who use maturity-based holdings can use those start years as the dates each bucket should be in place.", "GoodCallout"),
    p("Dashboard, Year Detail, and the main retirement report use prep-year language: a 2026 card can show bucket actions to prepare for 2027, so initial seed actions and later maintenance moves are visible where users review annual details. In the Bucket Lab tab, years before the setup-prep year show the start/seed years and inactive account previews, but no active bucket moves. The year before Initial Setup shows only seed/setup actions; the Initial Setup Year is the first operating year. The separate Bucket Lab Report separates start-of-year targets from actions to do during the Review Year to prepare the next year. Future Action Years starts after the coverage already shown, so the visible schedules add future move years without hiding the bucket coverage. Use Full Plan when you want to see all future move years.", "GoodCallout"),
...
story += [
    ...
    info_table(("Report Area", "Why It Is There"), [
        ...
        ("Bucket Lab", "Appears only when at least one account is enabled. The main report mirrors the Dashboard: actions in the report year prepare buckets for the following year. Use the separate Bucket Lab Report for detailed scheduled moves; its Future Action Years selector controls how much future ladder appears after the coverage rows."),
