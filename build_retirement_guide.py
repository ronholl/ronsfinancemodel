...
story += [
    ...
    p("A plain-English onboarding guide for setup, Roth Lab, Bucket Lab, check-ins, and advisor conversations", "GuideSub"),
    ...
    p("Quick translation: you do not need to understand every finance term before using the app. Build a reasonable baseline, use Readiness to find missing pieces, then use Roth Lab, Bucket Lab, or Strategy Builder only when you are ready to test choices.", "GoodCallout"),
    Spacer(1, 0.08 * inch),
    cards([
        ...
        ("Advisor use", "The Roth Lab and Bucket Lab report sections are designed to create better questions, not to replace professional advice."),
...
story += subhead("Three Active Planning Functions")
story += [
    p("Beyond tracking portfolio movement, the app actively models three decision systems."),
    cards([
...
story += [
    ...
    p("These taxable-account buckets are tax-treatment categories. They are separate from Bucket Lab, which is an optional portfolio-planning tool for withdrawal targets.", "GoodCallout"),
...
story += [
    p("Bucket Lab is an optional portfolio implementation tool. It turns the cash-flow plan into an investment-positioning checklist: what should be protected for near-term withdrawals, what should be staged for later withdrawals, and what can remain positioned for long-term growth?", "Callout"),
    p("The flow is Long-Term Growth -> Intermediate Bucket -> Short-Term Bucket -> Planned Withdrawals. The Dashboard and main report show actions during one year that prepare the buckets for the following year.", "GoodCallout"),
    p("Bucket Lab is neutral by default. Accounts start Off with no preset bucket window. Nothing appears in reports, the dashboard, or Year Detail until you turn on at least one account and enter the Short-Term / Intermediate years you actually want to use.", "GoodCallout"),
]
...
story += [
    info_table(("Bucket Lab Item", "Plain-English Meaning"), [
        ...
        ("On by account", "Turn on only the accounts where bucket targets are useful, then enter that account's Short-Term and Intermediate years. Leave an account Off when you do not want bucket targets for it."),
        ...
        ("Scheduled moves", "Shows money due to move between buckets. Refill ladders show the withdrawal year first, then a mechanical instruction: move to Short-Term by a start year, or stage in Intermediate by a start year and later move to Short-Term."),
        ...
        ("Bucket window", "The Short-Term / Intermediate year setting for one account. For example, Short-Term 2 years and Intermediate 5 years means two planned withdrawal years in Short-Term, then the next five in Intermediate. The app does not choose this for you."),
        ("Bucket Implementation", "The account-by-account funding tracker. It compares each spending year's calculated requirement with the money-market, Treasury, bond, CD, cash, or other holdings you record."),
        ("Funding status", "Not Funded, Partially Funded, Funded, or Overfunded. Green plan and implementation rows are funded; yellow rows are partially funded."),
    ]),
]
...
story += bullets([
    ...
    "It does not connect to a brokerage or infer trades. You record and maintain actual funding holdings yourself.",
])
story += subhead("Choosing Bucket Windows")
story += [
    p("There is no model-recommended bucket window. Choose the years with your advisor based on how conservative you want each account's withdrawal runway to be. Use 0 when a bucket should not exist.", "GoodCallout"),
    info_table(("Window Choice", "What It Means"), [
        ("Short-Term 0 / Intermediate 0", "No bucket targets for that account; it is effectively left in Long/Growth inside Bucket Lab."),
        ("Short-Term only", "Protect only the near-term planned withdrawals for that account."),
        ("Short-Term plus Intermediate", "Protect near-term withdrawals and stage later withdrawals so they can refill Short-Term as they get closer."),
        ("Different accounts", "Each enabled account can use its own window. A traditional IRA, inherited IRA, Roth, HSA, and taxable account do not need to match."),
    ]),
]
...
story += [simple_steps([
    ...
    ("4. Choose future action years", "The Future Action Years selector controls the scheduled move rows shown after the coverage tables. Coverage rows are always shown from the account's Short-Term and Intermediate windows. Choose No Future Actions for a shorter report, Five Years for the next future move window, or Full Plan for the whole remaining ladder."),
    ("5. Turn on accounts", "Enable only accounts where bucket targets should appear. Accounts are Off by default and have no preset window."),
    ("6. Set account windows", "Enter Short-Term and Intermediate years for each enabled account. Use zero when a bucket should not exist; do not treat any prior example as a recommendation."),
    ("7. Record actual funding", "In Bucket Implementation, add a holding, choose the IRA, HSA, taxable, inherited, or other enabled account it funds, and allocate an amount to one or more spending years. One holding may cover several years within the same account, but it never funds another account."),
    ("8. Read progress", "Required remains the calculated plan amount. Funded is the total recorded for that account-year, and Remaining is what still needs action. Green means funded; yellow means partially funded. Use By Year or By Account, expand or collapse groups, and click a funding source to edit it."),
    ("9. Move between plan and funding", "Use Go to Funding on an account plan card to jump directly to its implementation group. Use View Plan or the account name in implementation to return to the matching expanded plan card."),
    ("10. Review annually", "At each annual check-in, update balances, choose the new Review Year, and recheck implementation against the recalculated requirements. Recorded holdings remain attached to their account and year; the app does not automatically reassign them when the plan changes."),
])]
story += [
...
# Advisor report
story += section("9. Advisor Report")
story += [
    ...
    info_table(("Report Area", "Why It Is There"), [
        ...
        ("Bucket Lab", "Appears only when at least one account is enabled. The detailed Bucket Lab Report keeps coverage, actual implementation, funding status, and optional future scheduled moves together inside each account section."),
...
story += section("10. Annual Check-In And Strategy Builder")
story += [
...
# Reference setup
story += section("11. Setup Reference")
story += [
...
story += section("12. Inherited IRA Reference")
story += [
...
story += section("13. Advanced Mode, Backups, And Glossary")
story += [
